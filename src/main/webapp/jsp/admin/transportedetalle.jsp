								...
								<div class="col-lg-12">
									<div class="p-5">
										<div class="text-center">
											<h1 class="h4 text-gray-900 mb-4">Modificar un Transporte</h1>
										</div>
										<form action="/transporte/actualizar" method="post" class="user">
											<div class="form-group">
												<select name="conductor" class="form-select form-control" >													
													<c:forEach items="${ conductor }" var="co">
														<c:choose>
															<c:when test="${ transporte.idConductor.idConductor == co.idConductor }">
																<option value="${ co.idConductor }" selected>
																	<c:out value="${ co.rut }" />
																</option>
															</c:when>
															<c:otherwise>
																<option value="${ co.idConductor }" >
																	<c:out value="${ co.rut }" />
																</option>
															</c:otherwise>
														</c:choose>														
											...
											<div class="form-group">
												<select name="vehiculo"class="form-select form-control" >													
													<c:forEach items="${ vehiculo  }" var="ve">
														<c:choose>
															<c:when test="${ transporte.idVehiculo.idVehiculo == ve.idVehiculo }">
																<option value="${ ve.idVehiculo  }" selected >
																	<c:out	value="${ ve.patente }" />
																</option>
															</c:when>
															<c:otherwise>
																<option value="${ ve.idVehiculo }">
																	<c:out	value="${ ve.patente }" />
																</option>
															</c:otherwise>
														</c:choose>														
											...
											<div class="form-group row">
												<div class="col-sm-12 mb-6 mb-sm-0">
													<input type="number" placeholder="Precio Transporte" name="precio" required class="form-control form-control-user" value="${ transporte.precio}">
												</div>
											</div>
											
											
											<input type="hidden" value="${ transporte.idTransporte }" name="id">
