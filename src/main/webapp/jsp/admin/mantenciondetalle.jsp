								...
								<div class="col-lg-12">
									<div class="p-5">
										<div class="text-center">
											<h1 class="h4 text-gray-900 mb-4">Modificar un Mantenimiento</h1>
										</div>
										<form action="/mantencion/modificar" method="post" class="user" enctype="multipart/form-data">
											<div class="form-group">
												<select name="departamento" class="form-select form-control" >													
													<c:forEach items="${ departamento }" var="de">
														<c:choose>
															<c:when test="${ mantencion.idDepartamento.idDepartamento == de.idDepartamento }">
																<option value="${ de.idDepartamento }" selected>
																	<c:out value="${ de.idEdificio.nombre  } - ${ de.numero }" />
																</option>
															</c:when>
															<c:otherwise>
																<option value="${ de.idDepartamento }" >
																	<c:out value="${ de.idEdificio.nombre  } -${ de.numero }" />
																</option>
															</c:otherwise>
														</c:choose>														
											...
											<div class="form-group">
												...
												<input type="date" placeholder="Fecha" name="fecha" required class="form-control form-control-user" value="${ mantencion.fecha }" >												
											</div>
											<div class="form-group">
												<select name="Tipomantencion" class="form-select form-control" >													
													<c:forEach items="${ tipomantencion }" var="ti">
														<c:choose>
															<c:when test="${ mantencion.idTipomantencion.idTipomantencion == ti.idTipomantencion}">
																<option value="${ ti.idTipomantencion}" selected>
																	<c:out value="${ ti.descripcion  } " />
																</option>
															</c:when>
															<c:otherwise>
																<option value="${ ti.idTipomantencion }" >
																	<c:out value="${ ti.descripcion   }" />
																</option>
															</c:otherwise>
														</c:choose>														
											 ...
											 <div class="form-group row">
												<div class="col-sm-12 mb-6 mb-sm-0">
													<input type="number" placeholder="costo" name="costo" required class="form-control form-control-user" value="${ mantencion.costo }">
												</div>
											</div>		
												
											<input type="hidden" value="${ mantencion.idMantencion }" name="idMan">
